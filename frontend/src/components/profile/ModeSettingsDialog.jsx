...
export default function ModeSettingsDialog({ mode, isOpen, onClose }) {
  ...
  const renderContent = () => {
      if (mode === AvailabilityMode.ORANGE) {
          return (
              <div className="flex flex-col items-center py-4">
                  <div className="text-gray-600 mb-4">Maximum Contacts</div>
                  <Counter 
                      value={maxContacts} 
      ...
      if (mode === AvailabilityMode.YELLOW) {
          ...
          return (
              <div className="flex flex-col items-center py-4">
                  <div className="text-gray-600 mb-4">Duration (minutes)</div>
                  <Counter 
                      value={duration} 
      ...
      if (mode === AvailabilityMode.BROWN) {
          return (
              <div className="flex flex-col items-center py-4 gap-6">
                  <div>
                    <div className="text-gray-600 mb-2 text-center">Hour</div>
                    <Counter 
                        value={hour} 
                        onChange={setHour}
                  ...
                  <div>
                    <div className="text-gray-600 mb-2 text-center">Minute</div>
                    <Counter 
                        value={minute} 
                        onChange={setMinute}
      ...
      if (mode === AvailabilityMode.BLUE) {
          return (
              <div className="flex flex-col items-center py-4">
                  <div className="text-gray-600 mb-4">Available from date</div>
                  <Counter
                      value={0}
                      onChange={() => {}}
                      onIncrement={() => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md bg-white rounded-xl">
        <DialogHeader>
          <DialogTitle className="text-center">{getModeName()}</DialogTitle>
        </DialogHeader>
        ...
        <DialogFooter className="flex-row gap-2 justify-end">
          ...
          <Button onClick={handleSave} className="flex-1 bg-blue-600 hover:bg-blue-700">Done</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
